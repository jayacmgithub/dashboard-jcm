                    ...
                    <div class="m-l-10 align-self-center">
                        ...
                        if ($n_tgl_ini > 0) {
                            ...
                            <div id="userbox" class="userbox">
                                <?php if (($n_tgl_ini <= $tgl_proyek and level_user('proyek', 'data', $kategoriQNS, 'add') > 0) or ($n_tgl_ini > $tgl_proyek and $n_tgl_ini <= $tgl_kadiv and level_user('proyek', 'data', $kategoriQNS, 'add') > 0 and $proyek->getRow()->ijin_kadiv == 2) or ($n_tgl_ini > $tgl_kadiv and $n_tgl_ini <= $tgl_kadirat and level_user('proyek', 'data', $kategoriQNS, 'add') > 0 and $proyek->getRow()->ijin_kadirat == 2)) { ?>
                                    <!--<a class="btn btn-success" data-toggle="modal" data-target="#tambahData" style="font-size: 12px;color:white"> UPDATE ABSENSI</a>-->
                                    <a class="btn btn-success"
                                        href="<?= base_url() ?>proyek/import_mon_kry/<?= $proyek->getRow()->id_pkp ?>"
                    ...
                    <table cellspacing="0" id="table-basic" class="table table-sm table-bordered table-striped"
                        ...
                        <tbody>
                            ...
                            foreach ($detil_karyawan as $sol) {
                                ...
                                if ($nrp == '') {
                                    $nrp = 'Error';
                                    $nama_admin = 'NRP: ' . $sol->nrp . ', an ' . $sol->nama . ' belum ada di data pusat';
                                    $gaya_error = $error;
                                } else {
                                    $gaya_error = $biasa;
                                }
                    ...
                    <div class="form-group mt-lg proyek">
                        ...
                        <div class="col-sm-9">
                            <textarea type="text" name="sebab" class="form-control"
                                placeholder="Isi Sebab Keterlambatan" required></textarea>
                            <input type="hidden" name="id_pkp58" value="<?= esc($proyek->getRow()->id_pkp) ?>"
                                class="form-control" required />
